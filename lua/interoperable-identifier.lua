...
function decode(address, addressType, advType, elements)
  ...
  IBEACON_UUID_PREFIX = "\x49\x6f\x49\x44\x43\x4f\x44\x45\xb7\x3e"
  ...
  if manufacturerData ~= nil and #manufacturerData >= 25 then
    ...
    if companyCode ~= APPLE_COMPANY_CODE or
       ...
       iBeaconUuidPrefix ~= IBEACON_UUID_PREFIX then
      return handle
    end
  ...
  ----------------------------------
  if serviceData ~= nil and #serviceData >= 25 then
    -- TODO
  end

  -- No matching iBeacon or Eddystone-UID payload found
  return handle
end
